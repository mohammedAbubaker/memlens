...
TreemapNode add_directory_node(Treemap* treemap, int parent_index, std::filesystem::directory_entry dir_entry)
{
...
TreemapNode add_file_node(Treemap* treemap, int parent_index, std::filesystem::directory_entry dir_entry)
{
    TreemapNode node = add_node(treemap, dir_entry.path(), parent_index);
    treemap->leaves.insert(node.index);
    treemap->nodes.at(node.index).size = static_cast<double>(dir_entry.file_size());
    double_check_size += treemap->nodes.at(node.index).size;
    return node;
}
...
void visit_dir(Treemap* treemap, std::filesystem::path path, int parent_index)
{
    for (auto const& dir_entry : std::filesystem::directory_iterator { path }) {
        if (dir_entry.is_directory()) {
            TreemapNode node = add_directory_node(
                treemap,
                parent_index,
                dir_entry);
        ...
        if (dir_entry.is_regular_file()) {
            TreemapNode node = add_file_node(
                treemap,
                parent_index,
                dir_entry);
        }
...
void compute_sizes(Treemap* treemap)
{
    populate_parent_size(treemap, 0);
}
...
int main()
{
    ...
    double disparity = treemap.nodes.at(0).size - double_check_size;
